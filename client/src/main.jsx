...
function HomePage({ health, user, admin, announcements, onNavigate, onLogout }) {
  const statusText =
...
function PortalView({
  ...
  return (
    <Stack spacing={2.5}>
      ...
      {currentPage === "overview" && announcements.length > 0 ? (
        <AnnouncementTimeline announcements={announcements} />
      ) : null}
...
function DownloadConfigDialog({ open, onClose, baseUrl, apiKeys, defaultKey, models, onCopy }) {
...
function ModelHealthSection({ providers }) {
...
function normalizeModelFrontend(item) {
...
function AnnouncementTimeline({ announcements }) {
  ...
  return (
    <Paper variant="outlined" sx={{ p: { xs: 2, sm: 3 }, boxShadow: "0 12px 30px rgba(15, 23, 42, 0.05)" }}>
